...
const BookRepository = {
  ...
  async findPaginated(limit, offset) {
    ...
    const sql = `SELECT * FROM books ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
    const [rows] = await db.execute(sql);
    return rows;
  },
  ...
  async incrementAvailable(id) {
    const sql =
      "UPDATE books SET available_quantity = available_quantity + 1 WHERE id = ? AND available_quantity < quantity";
    const [result] = await db.execute(sql, [id]);
    return result.affectedRows > 0;
  },
};
